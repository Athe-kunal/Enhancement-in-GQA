...
def train(rank,world_size,model_name:str=config.MODEL_NAME):
    ...
    t5 = convert_t5_to_gqa(t5,kv_heads=4,similarity_flag=True)
    t5.to(rank)
    ...
    for epoch in range(config.NUM_EPOCHS):
        ...
        if rank == 0:
            ...
            torch.save(t5.module.state_dict(), f"simgqa_t5_finetuned_{epoch}.pth")
            artifact.add_file(f"simgqa_t5_finetuned_{epoch}.pth")
        # eval_dict_list = []
        # print(f"Started evaluation for epoch {epoch}")
        ...
        #     eval_dict_list.append(compute_metrics(eval_batch_pred_tensors.cpu(),eval_batch['labels'].cpu(),tokenizer,metric))
            ...
            wandb.log({f"simgqa_val_{epoch}_"+k:v for k,v in val_rouge_dict.items()})
            # wandb.log({f"val_rouge_{epoch}":val_rouge_dict})
        # break
    wandb.log({"simgqa_val_"+k:v[0] for k,v in val_rouge_dict.items()})
    if rank==0:
        test_dict_list = []
