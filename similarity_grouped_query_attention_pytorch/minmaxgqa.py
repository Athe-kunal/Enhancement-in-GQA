...
def mean_pool(key_or_value_heads,d_model,n_heads,kv_heads,h_dim,min_flag):
    ...
    #(n_heads(8), h_dim(64),d_model) -> (#kv_heads, #headspergroup, h_dim(64),d_model) and mean along axis 1
        key_or_value_heads = key_or_value_heads.view(kv_heads,n_heads//kv_heads,h_dim,d_model).min(axis=1).values
    else:
        key_or_value_heads = key_or_value_heads.view(kv_heads,n_heads//kv_heads,h_dim,d_model).max(axis=1).values
...
class CustomT5SelfAttention(T5Attention):
    def __init__(self, t5_decoder_attention_block,kv_heads:int,min_flag=False):
        ...
        modified_k_weights =  mean_pool(w_k,self.d_model,self.n_heads,self.kv_heads, self.key_value_proj_dim,min_flag)
        ...
        modified_v_weights = mean_pool(w_v,self.d_model,self.n_heads,self.kv_heads,self.key_value_proj_dim,min_flag)
...
def convert_t5_to_mgqa(module, kv_heads: int,inplace: bool = False,min_flag=False):
    ...
    for layer in out.decoder.block:
        layer.layer[0].SelfAttention = CustomT5SelfAttention(layer.layer[0].SelfAttention,kv_heads,min_flag)
        layer.layer[1].EncDecAttention  = CustomT5SelfAttention(layer.layer[1].EncDecAttention,kv_heads,min_flag)
...
if __name__=='__main__':
    ...
    for kv_heads in [4]:
        for min_flag in [False,True]:
            t5_gqa = convert_t5_to_mgqa(t5,kv_heads=kv_heads, inplace=False,min_flag=min_flag)

            t5_gqa.eval()

            outputs = t5_gqa.generate(input_ids, max_new_tokens=128)
            text = tokenizer.batch_decode(outputs[0], skip_special_tokens=True)
            print(f'Generated text with kv_heads:{kv_heads} and min flag:{min_flag} : {text}')